...
export function HorizontalBars({ items, width = 320, height, className }: HorizontalBarsProps) {
  const gradientId = `${useId().replace(/:/g, "")}-horizontal-bar-signal`;
  const rowHeight = 28;
          ...
          return (
            <Group key={item.label}>
              ...
              <Bar
                ...
                width={barW}
                height={barH}
                fill={item.color ?? `url(#${gradientId})`}
                rx={4}
              />
              <text
                x={innerW + 10}
