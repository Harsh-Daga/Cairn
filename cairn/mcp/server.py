"""Pillar 5 — minimal stdio MCP server (JSON-RPC over stdin/stdout)."""
...
from cairn.mcp.tools import ToolsContext, call_tool, list_tools, open_context
...
def serve(start_cwd: Path | None = None, *, stdin: Any = None, stdout: Any = None) -> int:
    ...
    start_cwd = start_cwd or Path.cwd()
    in_stream = stdin or sys.stdin
    out_stream = stdout or sys.stdout

    ctx = open_context(start_cwd)
    try:
