"""Pillar 3 — best-effort test runs (Part 11 + §2.7B)."""
...
def test_command_for(project: str | None) -> str | None:
    """Return the configured test command for *project*, or ``None``.

    Resolution: ``[tests] "<project>"`` → ``[tests] default`` → ``None``.
    """
...
def run_tests(cwd: str | None, project: str | None) -> TestResult:
    """Run the configured test command in *cwd*. Default OFF."""
...
def _parse_counts(stdout: str) -> tuple[int | None, int | None, int | None]:
    ...
    m = _PYTEST_RE.search(stdout)
    if m:
        passed = int(m.group(1))
        failed = int(m.group(2)) if m.group(2) else 0
        return passed, failed, passed + failed
    m = _JEST_RE.search(stdout)
    if m:
        passed = int(m.group(1))
        failed = int(m.group(2)) if m.group(2) else 0
        return passed, failed, passed + failed
    fm = _PYTEST_FAILED_RE.search(stdout)
    if fm:
        failed = int(fm.group(1))
        return 0, failed, failed
    if _GO_FAIL_RE.search(stdout):
        # Count failed go tests (best-effort).
